  ...
  <div class="space-y-6 animate-fade-in pb-20 md:pb-0">
    <div class="flex items-center justify-between mb-8">
      <div>
        ...
        <p class="text-kelola-sea mt-1 font-semibold text-sm">Catatan arus kasmu.</p>
      </div>
      <div class="hidden md:flex gap-3">
        <a href="/api/reports" download
          class="bg-white text-kelola-teal px-5 py-3 rounded-2xl font-bold shadow-[0_4px_15px_rgba(0,0,0,0.05)] hover:scale-105 transition-transform uppercase tracking-widest text-xs border-2 border-gray-100 hover:border-kelola-teal">
          ⬇ Excel
        </a>
        <NuxtLink to="/transactions/add"
          class="bg-kelola-pink text-white px-6 py-3 rounded-2xl font-black shadow-[0_0_20px_rgba(193,111,176,0.3)] hover:scale-105 transition-transform uppercase tracking-widest text-xs border border-transparent hover:border-white/20">
          + Tambah
        </NuxtLink>
      </div>
    </div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8" v-if="!pending && transactions?.length > 0">
      <div
        class="bg-white p-6 rounded-3xl shadow-[0_2px_10px_rgba(0,0,0,0.02)] border border-gray-50 flex flex-col items-center">
    ...
    <div class="flex flex-col md:flex-row md:items-center justify-between gap-4">
      <div class="flex gap-3 overflow-x-auto pb-2 scrollbar-hide snap-x flex-1">
        <button v-for="f in ['SEMUA', 'WAJIB', 'PENTING', 'TIDAK PENTING']" :key="f" @click="filter = f"
          ...
          :class="filter === f ? 'bg-kelola-teal text-kelola-lightgreen border-transparent shadow-lg scale-105' : 'bg-white text-gray-500 border-gray-200 hover:border-kelola-pink'">
      ...
      <div class="relative min-w-[180px]">
        <input type="month" v-model="selectedMonth"
          class="w-full bg-white border-2 border-gray-100 rounded-2xl px-4 py-2.5 text-sm font-bold text-kelola-teal focus:outline-none focus:border-kelola-teal transition-all shadow-sm cursor-pointer uppercase tracking-tight" />
      </div>
    </div>

    <!-- Loading State -->
    <div v-if="pending" class="text-center py-20 bg-white/50 rounded-3xl animate-pulse">
      <p class="text-kelola-sea font-bold">Memuat data...</p>
    </div>
        ...
        <div class="flex items-center gap-5">
          <div class="w-14 h-14 rounded-2xl flex items-center justify-center text-2xl shadow-inner relative"
            ...
            <div v-if="tx.receiptUrl"
              class="absolute -top-1 -right-1 bg-kelola-pink text-white w-5 h-5 rounded-full flex items-center justify-center text-[10px] shadow-sm font-bold">
              📄
            </div>
          </div>
          <div>
